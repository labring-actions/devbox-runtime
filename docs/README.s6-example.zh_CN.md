...
- `sdk-server` 在配置阶段会注册 s6 服务，但运行时还会判断：
  ...
  - 仅当同时满足以下条件时，才会改为以 `root` 用户运行：
    ...
    - `DEVBOX_SDK_RUN_AS_ROOT` 非空
  - `DEVBOX_SDK_RUN_AS_ROOT` 只用于显式切换 sdk-server 的运行用户；脚本当前按“是否非空”判断，不校验具体值，因此只要设置了非空值就会触发 root 运行。
  - 生产环境下即使设置了 `DEVBOX_SDK_RUN_AS_ROOT`，`sdk-server` 也不会启动。
...
- 除非确有必须使用 root 权限的场景，否则不要设置 `DEVBOX_SDK_RUN_AS_ROOT`。
