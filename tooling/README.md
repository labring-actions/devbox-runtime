...
These helper scripts under `tooling/scripts/` install small utilities used by the base images and runtime images (s6-overlay, cron runner, ssh proxy, etc.).
...
```bash
ARCH=x86_64 bash tooling/scripts/install-crond.sh
ARCH=arm64 bash tooling/scripts/install-s6.sh
ARCH=amd64 SSHPROXY_VERSION=v0.1.15 bash tooling/scripts/install-sshproxy.sh
```
