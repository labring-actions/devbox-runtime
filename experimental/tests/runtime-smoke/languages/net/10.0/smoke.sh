...
dotnet --version | grep -q '^10\.0'

if [ ! -f "$project_dir/hello_world.csproj" ]; then
  echo "Missing hello_world.csproj in $project_dir" >&2
  exit 1
fi

if [ ! -f "$project_dir/Program.cs" ]; then
  echo "Missing Program.cs in $project_dir" >&2
  exit 1
fi
