...
**Development mode:** For normal development environment, simply enter Devbox and run `bash entrypoint.sh` in the terminal. This will create a virtual environment, install Flask dependencies, and start the Flask development server with debug mode enabled for automatic reloading.

**Production mode:** After release, the project will be automatically packaged into a Docker image and deployed according to the `entrypoint.sh` script with production parameters (run `bash entrypoint.sh production`). This will create a virtual environment, install Flask and Gunicorn dependencies, and run the application with Gunicorn as a production WSGI server with worker processes for better performance and reliability.
