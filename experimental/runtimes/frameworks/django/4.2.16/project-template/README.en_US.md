...
**Development mode:** For normal development environment, simply enter Devbox and run `bash entrypoint.sh` in the terminal. This will create a virtual environment, install Django dependencies, and start the Django development server with increased verbosity for better debugging information.

**Production mode:** After release, the project will be automatically packaged into a Docker image and deployed according to the `entrypoint.sh` script with production parameters (run `bash entrypoint.sh production`). This will create a virtual environment, install Django dependencies, and start the Django application in a more production-like environment.
