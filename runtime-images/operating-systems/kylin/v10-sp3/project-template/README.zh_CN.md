...
该模板提供一个基于 Kylin Linux Advanced Server V10 SP3 的最小化**操作系统运行时**。
适用于需要国产化 Linux 基础环境，并在其上自行安装语言、框架或业务依赖的场景。

## 运行时概览

- 系统版本：`Kylin Linux Advanced Server V10 SP3`
- 基础运行时镜像：`kylin-v10-sp3`
- 启动脚本：`entrypoint.sh`
- 默认服务端口：`8080`
...
- 使用 `dnf` 或 `yum` 在该 Kylin 基础镜像中安装业务依赖。
- 保持容器暴露端口与服务监听端口一致。
